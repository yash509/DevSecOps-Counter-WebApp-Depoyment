pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Counter-WebApp-Depoyment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop cntwa | true"
                sh "docker rm cntwa | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name cntwa -p 3000:3000 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service cntwa-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"cntwa\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
